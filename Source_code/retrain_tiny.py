...
def load_allimages(dir):
    ...
    for root, _, fnames in sorted(os.walk(dir)):
        for fname in sorted(fnames):
            ...
            if datasets.folder.has_file_allowed_extension(fname,
            ...
            for idx, subset in enumerate(subset_list):
                ...
                for epoch in range(num_epochs):
                    ...
                    for batch_idx, (inputs, targets) in enumerate(train_loader):
                        ...
                        if (batch_idx + 1) % 50 == 0:
                            print(
                                f"Subset {idx}, Epoch [{epoch + 1}/{num_epochs}], Batch [{batch_idx + 1}/{len(train_loader)}], Loss: {loss.item():.4f}")

                    print(
                        f"Subset {idx}, Epoch [{epoch + 1}/{num_epochs}],Loss: {sum(epoch_loss) / len(epoch_loss):.4f}")
